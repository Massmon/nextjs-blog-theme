...
import {
  getNextResourceBySlug,
  getResourceBySlug,
  getPreviousResourceBySlug,
...
export const getStaticProps = async ({ params }) => {
  ...
  const { mdxSource, data } = await getResourceBySlug(params.slug);
  const prevResource = getPreviousResourceBySlug(params.slug);
  const nextResource = getNextResourceBySlug(params.slug);
...
export const getStaticPaths = async () => {
  const paths = getResourceFilePaths()
